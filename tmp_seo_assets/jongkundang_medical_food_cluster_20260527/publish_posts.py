# -*- coding: utf-8 -*-
"""Publish the Jongkundang medical-food cluster to Supabase posts."""
...
def main():
    ...
    if to_insert:
        try:
            inserted = request_json(f"{SB}/rest/v1/posts", method="POST", payload=to_insert, prefer="return=representation")
            results["inserted"] = [{"id": row.get("id"), "slug": row.get("slug"), "title": row.get("title")} for row in inserted]
        except urllib.error.HTTPError as err:
            results["error"] = {"status": err.code, "body": err.read().decode("utf-8", "replace")[:1200]}
