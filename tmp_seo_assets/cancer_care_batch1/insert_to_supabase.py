#!/usr/bin/env python3
"""Insert 16 cancer-treatment-care posts to Supabase (anon key + Accept-Profile public)."""
import json
import os
...
OUT_DIR = "/home/user/webapp/tmp_seo_assets/cancer_care_batch1"
os.makedirs(OUT_DIR, exist_ok=True)

print(f"[INSERT] {len(POSTS)} cancer-treatment-care posts → Supabase")
print(f"  endpoint: {SUPABASE_URL}/rest/v1/posts")
print()
...
for p in POSTS:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
            ...
            print(f"OK  id={rid:>4} slug={p['slug']}")
...
print(f"\n=== {ok}/{len(results)} OK ===")
if ok > 0:
    ok_ids = [x["id"] for x in results if x["status"] == "OK" and x.get("id")]
    if ok_ids:
        print(f"    id range: {min(ok_ids)} ~ {max(ok_ids)}")
