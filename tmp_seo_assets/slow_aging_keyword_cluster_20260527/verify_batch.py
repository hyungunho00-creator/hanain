# -*- coding: utf-8 -*-
"""Verify DB rows, image URLs, live blog pages, insight URLs, and sitemap presence."""
...
INSIGHT_SLUGS = [
    "slow-aging-keyword-roadmap-blood-sugar-sleep-protein",
    "slow-aging-checklist-meal-supplement-routine",
]
KEY = (
...
def main():
    ...
    results = {"db": db_rows(), "images": [], "pages": [], "insights": [], "sitemap_status": sitemap_status, "sitemap": []}
    for post in POSTS:
        slug = post["slug"]
        image = post["og_image"]
        try:
    ...
    for slug in INSIGHT_SLUGS:
        try:
            st, _, html = fetch(f"{SITE}/insights/{slug}", headers={"User-Agent": "Googlebot"})
            results["insights"].append({"slug": slug, "status": st, "html_has_bundle": "/assets/" in html})
        except Exception as exc:
            results["insights"].append({"slug": slug, "error": str(exc)})
        results["sitemap"].append({"url": f"/insights/{slug}", "present": f"/insights/{slug}" in sitemap})
