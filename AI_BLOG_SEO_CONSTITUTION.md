...
작성일: 2026-05-20
최종 개정: 2026-05-27 — 제20조 추가 (운동·병원정보 확장 및 동적 카운트 규칙)
...
## 제18조 (감태 연관키워드 클러스터 운영 규칙) — 2026-05-27 신설

감태 콘텐츠는 이미 대표 글이 많으므로 "감태 효능 총정리" 형태를 반복하지 않는다. 신규 글은 연관검색어의 검색 의도를 분리해 중복 SEO를 피한다.

1. 대표 키워드 반복 발행 금지
   - 이미 있는 대표 글: 감태 효능, 감태추출물, 감태 부작용, 감태 vs 미역·다시마, 감태추출물 vs 분말.
   - 새 글은 수면영양제, 디에콜, 요오드·갑상선, 먹는법, 가격, 후기, 차·분말·환·캡슐처럼 하위 의도로 쪼갠다.
2. 음식 감태와 감태추출물을 반드시 구분한다.
   - 생감태·건감태·감태차·감태분말은 식재료/가공식품 관점으로 쓴다.
   - 수면·디에콜·플로로탄닌 근거는 원물 반찬이 아니라 감태추출물 또는 표준화 원료 관점으로 쓴다.
3. 수면 콘텐츠는 의약품처럼 쓰지 않는다.
   - 감태추출물은 수면의 질 개선에 도움을 줄 수 있는 건강기능식품 원료로만 설명한다.
   - 불면증 치료, 수면제 대체, 즉시 효과, 장기 개선 보장 문구는 금지한다.
4. 디에콜 콘텐츠는 전임상과 인체 자료를 구분한다.
   - 디에콜은 플로로탄닌 계열의 대표 성분이지 플로로탄닌 전체와 같은 말이 아니다.
   - 2026년 동물·세포 연구를 사람 결과처럼 확대하지 않는다.
5. 요오드·갑상선·해조류 알레르기 주의문을 반복한다.
   - 갑상선 질환, 방사성요오드 치료, 저요오드 식이, 임신·수유, 12세 이하, 위장관 질환은 별도 확인 대상으로 둔다.
6. CTA는 성분표 점검형으로 작성한다.
   - 좋은 CTA: 제품 성분표, 섭취 목적, 복용 중인 약, 갑상선·수면 상태를 남기면 중복 축과 주의점을 정리할 수 있다는 안내.
   - 금지 CTA: 수면·혈당·면역 결과 보장, 강매형 구매 압박, 특정 제품 우위 단정.
7. 이미지도 검색 의도와 1:1로 대응한다.
   - 수면 글은 달·수면 리듬·캡슐, 디에콜 글은 성분표·분자, 요오드 글은 갑상선·해조류, 가격 글은 저울·라벨처럼 주제 상징을 다르게 쓴다.
   - 텍스트, 숫자, 로고, 실제 제품 패키지, 범용 바다 배경 재사용은 금지한다.

---

## 제19조 (확장 키워드·흥미도·SEO 구조 감사 규칙) — 2026-05-27 신설

새 콘텐츠는 단기 노출만 보지 않고 2차·3차로 확장 가능한 키워드 축을 먼저 고른다. 글과 이미지는 정보성만으로 끝내지 않고 독자가 계속 읽고 싶게 만드는 구조를 갖춰야 한다.

1. 확장 키워드는 1개 대표어 + 4개 이상 하위 의도로 설계한다.
   - 예: 저속노화 → 혈당, 수면, 단백질, 장건강, 항산화 폴리페놀.
   - 예: 감태 → 수면영양제, 디에콜, 요오드·갑상선, 먹는법, 가격, 후기.
2. 글은 첫 문단에서 사용자의 실제 불편감을 잡는다.
   - 좋은 시작: 식후 졸림, 새벽 각성, 부모님 식사량 감소, 성분표 혼란처럼 사용자가 바로 공감하는 장면.
   - 나쁜 시작: 원료 정의만 길게 반복하거나 논문 제목부터 나열.
3. 모든 글은 표·체크리스트·짧은 결론을 포함한다.
   - 독자가 훑어도 핵심을 얻어야 한다.
   - CTA는 자연스럽게 "내 상황을 남기면 정리해 드림" 구조로 둔다.
4. 이미지는 글의 검색 의도와 즉시 연결되어야 한다.
   - 혈당 글은 곡선·식사 순서, 수면 글은 달·리듬, 단백질 글은 식사·근육, 장건강 글은 장·미생물 상징.
   - 범용 바다, 범용 캡슐, 이전 이미지 재사용 금지.
5. SEO 구조 감사는 콘텐츠 발행 전후 모두 한다.
   - 카테고리 한글명 매핑: BlogPage, BlogPostPage, LandingPage, api/seo.js, generate_sitemap_rss.py.
   - sitemap/RSS 포함, canonical, og:image, og:image:alt, WebP MIME, robots 차단 여부를 확인한다.
   - 오류 발견 시 콘텐츠 발행과 같은 배포 단위에서 수정한다.

---

## 제20조 (운동·병원정보 확장 및 동적 카운트 규칙) — 2026-05-27 신설

운동법과 병원정보는 YMYL 민감도가 높고 검색 의도가 강하다. 특정 결과를 약속하거나 병원을 추천하는 방식이 아니라, 사용자가 안전하게 질문하고 기록할 수 있도록 돕는 정보형 구조로 작성한다.

1. 운동법 콘텐츠는 질환·치료 단계별로 나눈다.
   - 암환자 운동법, 항암치료 중 운동, 당뇨 운동법, 근감소증 운동법처럼 검색 의도를 분리한다.
   - 운동 강도보다 중단 신호, 의료진 상담이 필요한 상황, 다음 날 회복 여부를 먼저 쓴다.
2. 병원정보 콘텐츠는 추천·순위가 아니라 체크리스트로 작성한다.
   - 암 재활병원, 암 병원 정보, 당뇨 교육 병원, 병원 진료 전 질문 리스트처럼 역할을 나눈다.
   - 특정 병원 우위, 치료 결과 보장, 순위성 표현은 금지한다.
3. 운동·병원정보는 식사·단백질·ONS·환자식과 자연스럽게 연결한다.
   - 구매 압박이 아니라 "진단명, 치료 단계, 식사량, 체중 변화, 복용약, 운동 가능 시간"을 정리하도록 유도한다.
   - 플로로탄닌은 운동 성분처럼 말하지 않고 항산화·폴리페놀 정보와 성분표 점검 축으로만 연결한다.
4. 새 카테고리는 Supabase `categories` 테이블과 코드 매핑을 동시에 맞춘다.
   - BlogPage fallback, LandingPage preview, api/seo.js, generate_sitemap_rss.py를 함께 수정한다.
   - Supabase `categories`에 INSERT/확인하지 않으면 운영 탭에서 누락될 수 있다.
5. 콘텐츠 개수 표시에는 하드코딩 숫자를 쓰지 않는다.
   - 인사이트 개수는 `INSIGHTS_LIST.length`처럼 실제 데이터 길이를 사용한다.
   - "60편"처럼 과거 수량이 남아 있으면 UI·메타·CTA·주석까지 검색해서 제거한다.

---

## 제21조 (포티멜 제외 및 맛있으리·플로로탄닌 CTA 규칙) — 2026-05-27 신설

이후 신규 콘텐츠와 CTA에서는 포티멜을 기본 노출 축에서 제외한다. 환자식·ONS·특수의료용도식품 콘텐츠는 맛있으리 식단관리와 플로로탄닌 건강정보 CTA만 사용한다.

1. 신규 블로그, 인사이트, 정적 SEO 문구, 이미지 alt, CTA에는 포티멜을 넣지 않는다.
   - 예외: 사용자가 기존 포티멜 글의 수정·삭제·팩트체크를 명시적으로 지시한 경우.
   - 기존 색인 URL이나 기존 포티멜 주제 글은 사용자 지시 없이 삭제하지 않는다.
2. CTA는 맛있으리와 플로로탄닌으로만 구성한다.
   - 좋은 CTA: 진단명, 치료 단계, 식사량, 혈당 기록, 복용약, 성분표를 남기면 맛있으리 식단관리와 플로로탄닌 건강정보 관점에서 확인 기준을 정리한다.
   - 금지 CTA: 경쟁 제품명 반복, 특정 제품 비난, 질환 개선 보장, 구매 압박.
3. 경쟁사·타사 제품을 다룰 때는 사실 확인형 정보로만 쓴다.
   - 공식 제품 페이지, 식품안전나라, 학회·공공기관 자료를 우선한다.
   - "배신", "나쁘다", "효과 없다"처럼 감정적·비방성 표현은 본문과 메타에서 쓰지 않는다.
4. 특수의료용도식품은 치료제가 아니라 영양 보충·식사 보완 도구로 설명한다.
   - 암·당뇨·신장질환·삼킴 문제·수술 전후 글에는 의료진 또는 임상영양사 상담 필요 문구를 포함한다.
5. 이미지에는 실제 제품 패키지, 로고, 텍스트, 숫자를 넣지 않는다.
   - 당뇨음료 글은 혈당 기록·식사·성분표 상징, 암환자 음료 글은 식욕부진·삼킴·보호자 기록 상징으로 만든다.
