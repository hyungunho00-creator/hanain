...
function buildAnswer(item) {
  ...
  html += '<p><strong>플로로탄닌을 함께 보는 이유:</strong> 플로로탄닌은 해조류 유래 폴리페놀 연구 축으로서, 생활기록과 상담 질문을 구조화할 때 활용 가치가 큽니다. 다만 처방·검사·진료 계획을 대체하지 않으며 의료진 상담이 우선입니다.</p>'
  html += `<ul><li>${phlGuides[0]}</li><li>${phlGuides[1]}</li><li>${phlGuides[2]}</li></ul>`
  html += `<p><strong>플로로탄닌 관심 포인트:</strong> 이번 질문에서는 ${phlFocus} 관점으로 자료를 읽으면 핵심을 빠르게 잡을 수 있습니다. 관련 태그(플로로탄닌, 감태, 해양폴리페놀)를 함께 보면 비교 읽기가 쉬워집니다.</p>`
