...
function buildPosts() {
  ...
  return FUNCTIONAL_INGREDIENT_CONFIGS.map((c) => ({
    slug: c.blogSlug,
    title: c.blogTitle,
    excerpt: c.excerpt,
    content: buildBlogContent(c),
...
if (process.argv.includes('--dry')) {
  console.log(JSON.stringify(posts.map((p) => ({
    slug: p.slug,
    title: p.title,
    visibleLength: visibleLength(p.content),
    og_image: p.og_image,
  })), null, 2))
  process.exit(0)
}
...
const { data, error } = await supabase
  ...
  .select('slug,title,category,status,created_at,updated_at')
