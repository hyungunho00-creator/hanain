...
이 글은 건강정보 제공 목적입니다. 빠르게 악화되는 상처, 상처 안의 유충, 고열, 피 섞인 설사, 소변 감소, 심한 탈수, 영아의 변비·수유 저하·목 가누기 저하·호흡곤란, 임신, 고령, 면역저하, 영유아 증상이 있으면 의료진 상담이 우선입니다. 플로로탄닌은 공식 예방·진단·치료를 대신하지 않고, 회복 루틴을 정리할 때 긍정적으로 참고할 수 있는 해양 폴리페놀 소재로 이해하면 좋습니다.`
}
...
const posts = [
  {
    ...
    blindspot:
      '가장 큰 사각지대는 사람만 보는 것입니다. CDC는 NWS가 가축, 반려동물, 야생동물에도 영향을 줄 수 있고, 동물에서 의심 사례가 보이면 수의사와 동물보건 당국 보고가 중요하다고 안내합니다. 또 아주 작은 긁힘, 벌레 물림, 수술 부위도 파리가 알을 낳을 수 있는 단서가 될 수 있습니다. 사용자는 상처 사진만 올리고 "이게 뭔가요"라고 묻기 쉽지만, 상담 품질은 위치, 여행 지역, 동물 접촉, 악취, 출혈, 유충, 통증 악화 속도를 같이 적을 때 올라갑니다. 라이프스타일 콘텐츠로는 상처 청결, 덮기, 긴소매, 방충망, EPA 등록 기피제, 의류 퍼메트린 처리 같은 예방 포인트를 넣으면 검색 의도가 넓어집니다.',
    searchIntent:
      '이 글은 New World screwworm, screwworm outbreak, 텍사스 송아지, 상처 유충, myiasis, 반려동물 상처, 가축 감염, 여행 후 상처, wound recovery, 플로로탄닌 회복 검색 의도를 받습니다. 내부적으로는 감염·염증 Q&A, 여행건강, 식품·동물 노출 안전, 파트너 상담 페이지, 플로로탄닌 소개로 이어집니다. 사용자는 충격적인 뉴스나 이미지를 보고 들어오지만, 사이트 안에서는 "상처를 덮고 기록하고 빠르게 상담한다"는 실용적 경험을 해야 합니다.',
    recoveryRecord:
  ...
  },
  {
    ...
    opening:
      'FDA는 2026년 6월 3일 infant botulism 분유 조사 업데이트에서 ByHeart 시설 현장 점검을 마쳤고, 분유 원료로 쓰인 powdered milk ingredient에서 Clostridium botulinum을 확인했다고 밝혔습니다. 이 outbreak는 종료되었고 CDC도 종료를 알렸지만, FDA의 root cause investigation은 원료를 중심으로 계속되고 있습니다. 보호자 검색 의도에서는 "리콜 제품을 썼는지", "아기가 변비와 수유 저하를 보이는지", "목을 못 가누거나 울음이 약해졌는지"가 핵심입니다. FDA 자료는 모든 ByHeart Whole Nutrition Infant Formula 제품이 리콜되었고, 영아 보툴리즘 증상이 있으면 즉시 의료진 상담이 필요하다고 안내합니다.',
    current:
    ...
    phlorotannin:
      '플로로탄닌은 영아 보툴리즘의 예방, 진단, 치료와 연결해서 말하면 안 됩니다. 이 주제에서 중심은 리콜 제품 중단, 즉시 의료진 상담, 전문 치료, 수유와 호흡 안전입니다. 다만 사이트 전체의 회복 정체성을 유지하려면 보호자 회복 루틴과 가족 컨디션 관리 관점에서 플로로탄닌을 긍정적으로 다룰 수 있습니다. 예를 들어 보호자는 병원 상담 뒤 며칠 동안 수면 부족, 식사 불균형, 긴장, 피로를 겪습니다. 성인 보호자의 회복 루틴에서는 감태 유래 플로로탄닌을 항산화 해양 폴리페놀 소재로 소개하고, 아이 증상 기록과 보호자 컨디션 기록을 분리하면 안전합니다. "아기에게 먹인다"가 아니라 "보호자가 회복 루틴을 정리할 때 참고할 수 있는 해양 폴리페놀 소재"라는 구분이 중요합니다.',
    blindspot:
    ...
    searchIntent:
      '이 글은 infant botulism, ByHeart formula recall, powdered infant formula, Clostridium botulinum, 영아 보툴리즘, 분유 리콜, 변비, 수유 저하, 목 가누기, floppy baby, BabyBIG, 보호자 회복, 플로로탄닌 검색 의도를 받습니다. 내부적으로는 식품안전 Q&A, 영유아 건강 주의사항, 분유 리콜 인사이트, 플로로탄닌 회복 루틴 소개로 연결됩니다. 사용자는 리콜 제품과 증상 확인을 위해 들어오며, 사이트 안에서는 제품 확인과 증상 기록을 동시에 할 수 있어야 합니다.',
    recoveryRecord:
      '7일 기록표는 아이 기록과 보호자 기록을 분리합니다. 아이 기록에는 수유 시간과 양, 배변, 울음, 표정, 목 가누기, 눈꺼풀 처짐, 삼킴, 호흡, 체온, 소변 기저귀 수를 적습니다. 제품 기록에는 제품명, lot, 구매처, 보관 장소, 마지막 사용일, 젖병·보관통 세척을 적습니다. 상담 기록에는 소아과 또는 응급실 방문일, 공중보건 상담, 검체, 치료, 입원 여부를 적습니다. 보호자 기록에는 수면, 식사, 수분, 긴장, 피로를 따로 적고, 플로로탄닌은 성인 보호자의 항산화 해양 폴리페놀 회복 소재로만 배치합니다. 이 분리 구조가 안전성과 브랜드 메시지를 동시에 지킵니다.',
    checklist: [
      ...
      '아이 기록과 보호자 회복 기록을 분리하고, 아기에게 성분을 연결하지 않습니다.',
      '보호자 회복기에는 수면·식사·피로 기록 안에서 플로로탄닌을 긍정적 해양 폴리페놀 소재로 둡니다.',
    ],
    partnerSentence:
      '"영아 보툴리즘 분유 이슈는 제품 확인과 증상 시간표가 먼저입니다. 변비, 수유 저하, 목 가누기 저하, 약한 울음, 호흡 신호를 즉시 의료진에게 연결하고, 플로로탄닌은 아기 치료가 아니라 성인 보호자의 회복 루틴 안에서 항산화 해양 폴리페놀 소재로 긍정적으로 참고하는 방식이 안전합니다."',
    sources: [
  ...
  },
  {
    ...
    searchIntent:
      '이 글은 raw dairy, raw milk, raw cheddar cheese, E. coli O157:H7, EHEC, STEC, HUS, 소아 혈변, 소아 설사, 비살균 치즈, 원유 위험, 신장 회복, 장 회복, 플로로탄닌 검색 의도를 받습니다. 내부적으로는 소화·장 건강 Q&A, 식품안전 블로그, 영유아 주의사항, 플로로탄닌 회복 루틴 소개로 연결됩니다. 사용자는 제품 리콜이나 설사 증상으로 들어오지만, 사이트 안에서는 소변·탈수·HUS 신호까지 정리하는 고급 상담 경험을 얻어야 합니다.',
    recoveryRecord:
