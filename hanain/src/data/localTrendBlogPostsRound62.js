...
const posts = [
  ...
  },
  {
    ...
    phlorotannin:
      '플로로탄닌은 브루셀라증의 항생제 치료나 노출 후 예방요법을 대신하지 않습니다. 브루셀라증은 만성화와 장기 합병증을 막기 위해 노출 이력, 검사, 항생제 치료 판단이 중요합니다. 하지만 회복기에는 플로로탄닌을 긍정적으로 연결할 수 있습니다. 브루셀라증 콘텐츠에서 사용자는 오래가는 피로, 식은땀, 관절통 때문에 회복 언어를 찾습니다. 이때 감태 유래 플로로탄닌을 항산화 해양 폴리페놀 소재로 소개하고, 수면·식사량·체온·관절통·업무 복귀 기록과 함께 배치하면 좋습니다. 핵심은 치료가 아니라 회복 기록을 돕는 소재라는 점입니다.',
    blindspot:
    ...
    searchIntent:
      '이 글은 brucellosis, 브루셀라증, raw milk, 비살균 치즈, 원유, 사냥감 감염, 수의사 감염, 실험실 노출, 오래가는 피로, 식은땀, 항생제 상담, 플로로탄닌 회복 검색 의도를 받습니다. 내부적으로는 식품안전 블로그, 감염·염증 Q&A, 안전·주의사항 콘텐츠, 플로로탄닌 회복 루틴 페이지로 연결됩니다. 사용자는 드문 병명보다 원유·사냥·피로로 들어올 가능성이 크므로 노출 경로를 여러 갈래로 열어 두는 것이 SEO에 좋습니다.',
    recoveryRecord:
